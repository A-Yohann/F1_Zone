...
#[AsCommand(
    name: 'app:import-courses-csv',
    description: 'Importe les courses depuis un fichier CSV dans la base de données.'
)]
class ImportCoursesCsvCommand extends Command
{
    ...
    {
        $file = __DIR__ . '/../../var/data/courses.csv';
        if (!file_exists($file)) {
            $output->writeln('<error>Fichier courses.csv introuvable dans var/data.</error>');
        ...
        while (($row = fgetcsv($handle)) !== false) {
            ...
            if (!$pilote) {
                $output->writeln("<comment>Pilote non trouvé : {$data['vainqueur']} (nom: $nom, prenom: $prenom)</comment>");
                continue;
            }

            // Recherche de l'écurie
            $ecurie = $this->entityManager->getRepository(Ecurie::class)->findOneBy([
                'nom' => $data['ecurieVainqueur'],
            ]);
            if (!$ecurie) {
                $output->writeln("<comment>Ecurie non trouvée : {$data['ecurieVainqueur']}</comment>");
                continue;
            }
        ...
        $output->writeln("<info>$count courses importées avec succès !</info>");
